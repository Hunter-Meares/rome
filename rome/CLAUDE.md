...
The first real test file was `world/tests_chargen.py`, covering `_leans_caster` and the chargen synergy-hint mismatch logic - chosen first because it's pure logic with zero database dependency, and because it had already had two real, separate bugs found by hand (not by any automated test), making it the highest-value place to lock in regression coverage first. It's since grown into a full suite: 12 test files (`tests_chargen.py`, `tests_combat.py`, `tests_combat_commands.py`, `tests_party.py`, `tests_achievements.py`, `tests_underworld.py`, `tests_items.py`, `tests_economy.py`, `tests_colosseum.py`, `tests_npcs.py`, `tests_quit.py`, `tests_social.py`), **244 tests total, all passing** (one intentional skip). Read `tests_chargen.py` as the pattern for plain-`unittest`-style tests (no `EvenniaTest` needed) when testing pure calculation/logic functions elsewhere in the codebase.
