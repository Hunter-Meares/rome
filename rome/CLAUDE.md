...
11. **The same bare-typeclass issue from gotcha #10 also affects Characters, not just scenery Objects.** Found live during Campus Martius's post-build verification: the Pantheon's own priest NPC (created in the original Pantheon build, untouched by any later change) is `evennia.objects.objects.DefaultCharacter` rather than `typeclasses.characters.Character`, so an `is_typeclass("typeclasses.characters.Character", exact=False)` check silently misses it entirely - it looked absent from the room when it was actually just built with the wrong base class. The NPC itself functions fine (this doesn't break anything the way the Object bug did), but any future live-verification script checking "does this room have its expected NPC" needs to check against `evennia.objects.objects.DefaultCharacter` too, or use a broader/duck-typed check, or it will report real NPCs as missing. Not yet swapped like the 21 Objects were - flagged here rather than fixed blindly, since swapping a Character's typeclass live carries more risk (scripts, combat state) than a scenery prop.
