"""
One-time live setup for the Domus Aurea (Nero's Golden House): creates
all 18 rooms and 18 exits from world/batch_domus_aurea_data.py (17
palace rooms + the 1-room geographic-accuracy connector), attaches to
the real, already-built "The Meta Sudans" via its previously-unused
"south" exit, and populates the flavor NPCs and lookable scenery
objects.

Run once via `evennia shell < world/setup_domus_aurea_live.py`. Not
idempotent - re-running duplicates everything.
"""

from evennia.utils import search, create

from world.batch_domus_aurea_data import ROOMS, LINKS, NPCS, OBJECTS, ECHOES

CHARACTER_TYPECLASS = "typeclasses.characters.Character"
OBJECT_TYPECLASS = "typeclasses.objects.Object"

# ----------------------------------------------------------------------
# 1. Create all 18 rooms
# ----------------------------------------------------------------------
...
for key, data in ROOMS.items():
    ...
    room.tags.add("domus_aurea", category="zone")
    room_objs[key] = room
...
anchors = search.search_object("The Meta Sudans", typeclass="typeclasses.rooms.Room")
if not anchors:
    raise SystemExit("ABORTED: could not find the real 'The Meta Sudans' room live.")
anchor_room = anchors[0]

exit_count = 0
for from_key, from_dir, to_key, to_dir in LINKS:
    from_room = anchor_room if from_key == "existing_meta_sudans" else room_objs[from_key]
    to_room = anchor_room if to_key == "existing_meta_sudans" else room_objs[to_key]
...
print("Created %d exits (including the connector to The Meta Sudans)." % exit_count)
...
print("Domus Aurea setup complete: %d rooms, %d exits, %d NPCs, %d objects." % (
