"""
One-time live setup for the Bibliotheca Ulpia (Library of Rome):
creates all 16 rooms and 16 exits from world/batch_library_data.py
(15 library rooms + the 1-room geographic-accuracy connector),
attaches to the real, already-built "Market Rooftop Terrace" via its
previously-unused "down" exit, and populates the flavor NPCs and
lookable scenery objects.

Run once via `evennia shell < world/setup_library_live.py`. Not
...
for key, data in ROOMS.items():
    ...
    room.tags.add("library", category="zone")
    room_objs[key] = room
...
anchors = search.search_object("Market Rooftop Terrace", typeclass="typeclasses.rooms.Room")
if not anchors:
    raise SystemExit("ABORTED: could not find the real 'Market Rooftop Terrace' room live.")
anchor_room = anchors[0]

exit_count = 0
for from_key, from_dir, to_key, to_dir in LINKS:
    from_room = anchor_room if from_key == "existing_market_rooftop_terrace" else room_objs[from_key]
    to_room = anchor_room if to_key == "existing_market_rooftop_terrace" else room_objs[to_key]
...
print("Created %d exits (including the connector to Market Rooftop Terrace)." % exit_count)
...
print("Library setup complete: %d rooms, %d exits, %d NPCs, %d objects." % (
